use std::{env, fmt::Display, net::SocketAddr, process::ExitCode, sync::{mpsc::{self, Sender}, Arc}, thread::{self, JoinHandle}, time::Duration};
...
fn get_master_from_sentinel(connection: &mut Connection, master_name: &str) -> Result<RedisAddr, Error> {
    ...
    if response.len() != 2 {
        return Err(Error::InvalidResponse("Response did not have exactly 2 elements!".to_owned()))
    }
...
fn listen_for_master_switches(client: Arc<Client>, sender: Sender<RedisAddr>, master_name: &str) -> JoinHandle<()> {
    ...
    return thread::spawn(move || {
        loop {
            ...
            let subscribe_result = connection.subscribe::<_, _, ()>(topic, |msg| {
                ...
                let segments: Vec<&str> = value.as_str().split_ascii_whitespace().into_iter().collect();
                if segments.len() < 5 {
                ...
                if master_name.as_str() != affected_master {
                    println!("Master changed for {}, we are not interested in that...", affected_master);
                    return ControlFlow::Continue;
                }
...
fn poll_master_address(client: Arc<Client>, sender: Sender<RedisAddr>, master_name: &str, poll_interval: &Duration) -> JoinHandle<()> {
    let master_name = master_name.to_string();
...
fn main() -> ExitCode {
    ...
    if args.len() != 4 {
        ...
        eprintln!("Usage: {} <sentinal host:port> <master name> <poll interval secs>", args[0]);
    ...
    let _ = poll_master_address(client.clone(), tx.clone(), master_name.as_str(), &poll_interval);
